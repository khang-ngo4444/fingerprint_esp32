...
#include <base64.h>       // base64 cho Arduino/ESP32
...
// Nếu chưa có, cài thư viện base64 cho Arduino/ESP32
void sendFingerprintTemplateToServer(int id)
{
  uint8_t templateBuffer[512];
  int templateLength = finger.downloadModel(id, templateBuffer);
  if (templateLength <= 0)
  {
    Serial.println("Không thể đọc template từ sensor!");
    return;
  }
  String templateBase64 = base64::encode(templateBuffer, templateLength);
  ...
  char buf[1024];
  size_t n = serializeJson(doc, buf);
  mqtt.publish("fingerprint/template", buf, n); // hoặc TOPIC_SCAN nếu muốn
  Serial.println("Đã gửi template vân tay lên server!");
}